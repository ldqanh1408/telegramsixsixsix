...
GitHub gửi POST /github/webhook/my_repo_bot
  ...
  ▼
GitHubWebhookController (thin adapter)
  → GitHubWebhookProcessor.process(...)
      → bots.findEnabled("my_repo_bot")
      → WebhookSignatureVerifier.verify(secret, signature, body)  // HmacSha256 adapter
      → matchesConfiguredRepo(bot, payload)                       // so sánh repo
      → notifications.dispatch(bot, "push", payload)
      → trả GitHubWebhookResult(OK, "ok") → controller map sang HTTP 200
    ▼
NotificationService
...
| GitHub webhook | HMAC-SHA256 signature | `WebhookSignatureVerifier` port → `HmacSha256SignatureVerifier` |
