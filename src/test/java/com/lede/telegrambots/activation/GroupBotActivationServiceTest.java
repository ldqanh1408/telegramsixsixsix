...
class GroupBotActivationServiceTest {
    ...
    void activateReusesInactiveRecordAndMarksItActive() {
        GroupActivationRepository activations = mock(GroupActivationRepository.class);
        ...
        when(activations.findByBotUsernameAndChatId("helper_bot", -100123L))
                .thenReturn(Optional.of(inactive));
        when(activations.save(any(GroupActivation.class))).thenAnswer(invocation -> invocation.getArgument(0));

        GroupBotActivationService service = new GroupBotActivationService(activations);
